...
- Прокси читает `/proc/net/tcp` и `/proc/net/tcp6`. Прокси ищет сокет в состоянии `LISTEN`
  на порту `UPSTREAM_PORT`.
- Это обычное чтение таблицы сокетов ядра. Проверка ничего не отправляет в целевой API.
- Прокси отвечает 200 с `{"status":"ok"}`, если хотя бы одна из таблиц показывает слушающий сокет.
- Прокси отвечает 503 с `{"status":"upstream unreachable"}`, если ни одна не показывает
  слушающий сокет.
- Прокси переходит к TCP-соединению, если ни один из файлов не читается. Так бывает на хосте
  не на Linux или в ограниченном окружении.
- Прокси открывает TCP-соединение к локальному порту stunnel. Таймаут — 1 секунда. Прокси также
  пишет одну строку `WARNING`.
- `/healthz` не пишет других строк в лог.
...
- `GET /healthz` молчит на любом другом уровне, кроме строки `WARNING` при переходе к TCP-соединению. На DEBUG он также пишет `Received http request` и `api operation executed`.
